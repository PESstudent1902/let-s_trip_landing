...
import { X, Calendar, MapPin, CheckCircle2, ChevronRight, Clock, Loader2 } from "lucide-react";
import FallbackImage from "./FallbackImage";
import { Package, Destination, ItineraryDay } from "@/lib/packageStore";
import { useState, useEffect } from "react";
import { fetchPackageDetail } from "@/app/actions";
...
export default function ItineraryModal({ pkg, isOpen, onClose, destination }: ItineraryModalProps) {
  ...
  const [crmItinerary, setCrmItinerary] = useState<ItineraryDay[] | null>(null);
  ...
  // Fetch CRM itinerary when modal opens for CRM packages
  useEffect(() => {
    if (isOpen && pkg && pkg.id.startsWith("crm-")) {
      setIsLoadingItinerary(true);
      setCrmItinerary(null);
      fetchPackageDetail(pkg.id).then((detail) => {
        if (detail?.itinerary && detail.itinerary.length > 0) {
          setCrmItinerary(detail.itinerary);
        }
  ...
  // Use CRM itinerary if available, fall back to local
  const itinerary = crmItinerary || pkg.itinerary || [];
              ...
              <div className="flex flex-col md:flex-row h-full overflow-y-auto md:overflow-hidden" data-lenis-prevent>
                ...
                <div className="w-full md:w-2/5 flex flex-col relative flex-shrink-0">
                  <div className="relative h-64 md:h-1/2 w-full">
                    <FallbackImage src={pkg.image} alt={pkg.name} fallbackName={pkg.name} fill className="object-cover" />
                  ...
                  <div className="space-y-4 relative">
                    ...
                    {isLoadingItinerary ? (
                      <div className="text-center py-10 glass rounded-2xl">
                        <Loader2 className="mx-auto text-cyan mb-3 animate-spin" size={32} />
                        <p className="text-text-secondary" style={{ fontFamily: "var(--font-handwritten)" }}>
                          Loading itinerary from CRM...
                        </p>
                      </div>
                    ) : itinerary.length > 0 ? (
                      itinerary.map((day, i) => (
