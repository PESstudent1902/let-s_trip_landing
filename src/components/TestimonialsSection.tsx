...
import { Star, Quote, Shield, Award, Heart, ThumbsUp } from "lucide-react";
...
export default function TestimonialsSection() {
  ...
  return (
    <section ref={ref} id="reviews" className="relative py-32 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-6 md:px-16">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center mb-16">
          <span className="inline-flex items-center gap-2 px-4 py-2 glass rounded-full text-gold text-xs font-semibold tracking-[0.15em] uppercase mb-6" style={{ fontFamily: "var(--font-headline)" }}>
            <Star size={14} fill="currentColor" /> Trusted by Thousands
          </span>
          <h2 className="text-4xl md:text-6xl font-bold mb-4" style={{ fontFamily: "var(--font-headline)" }}>
            What Travelers <span className="bg-gradient-to-r from-gold via-orange to-violet bg-clip-text text-transparent">Say</span>
          </h2>
          <p className="text-text-secondary text-lg max-w-xl mx-auto">Real stories from real adventurers who trusted LetsTrip</p>
        </motion.div>
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
          {testimonials.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.1 }}
              className="break-inside-avoid glass rounded-2xl p-6 hover:border-cyan/20 transition-all duration-300 group">
              {/* Quote Icon */}
              <Quote className="text-cyan/20 mb-3" size={28} />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan to-violet flex items-center justify-center text-white text-xs font-bold" style={{ fontFamily: "var(--font-headline)" }}>
                    {t.name.split(" ").map(n => n[0]).join("").slice(0, 2)}
                  </div>
                  <div>
                    <p className="font-semibold text-sm" style={{ fontFamily: "var(--font-headline)" }}>{t.name}</p>
                    <p className="text-text-muted text-xs">{t.sub}</p>
                  </div>
                </div>
                <span className={`px-3 py-1 rounded-full text-[10px] font-semibold tracking-wider uppercase ${t.tagColor}`} style={{ fontFamily: "var(--font-headline)" }}>
                  {t.tag}
                </span>
              </div>
            </motion.div>
        ...
        {/* Trust Badges */}
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.8 }}
            ...
            {trustBadges.map((badge) => (
              <div key={badge.label} className="flex flex-col items-center text-center gap-3">
                <div className="p-3 rounded-xl bg-white/5">
                  <badge.icon size={24} className="text-cyan" />
                </div>
                <p className="text-sm font-semibold text-text-secondary" style={{ fontFamily: "var(--font-headline)" }}>{badge.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
