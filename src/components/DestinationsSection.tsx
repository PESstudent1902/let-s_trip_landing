    ...
    <section ref={ref} id="destinations" className="relative py-20 md:py-32 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-16">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="text-center mb-10 md:mb-16">
          ...
          <h2 className="text-4xl sm:text-5xl md:text-7xl mb-3 md:mb-4" style={{ fontFamily: "var(--font-brush)" }}>
            ...
            <span className="bg-gradient-to-r from-cyan to-violet bg-clip-text text-transparent text-glow-cyan">Experiences</span>
          </h2>
          <p className="text-text-secondary text-base md:text-lg max-w-xl mx-auto" style={{ fontFamily: "var(--font-handwritten)" }}>Destinations that defy gravity and exceed expectations</p>
        </motion.div>

        {/* Destination Cards — Carousel on desktop, stacked on mobile */}
      ...
      <div id="packages" className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 md:px-16 mt-20 md:mt-32">
        <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} className="text-center mb-10 md:mb-16">
          ...
          <p className="text-text-secondary text-base md:text-lg" style={{ fontFamily: "var(--font-handwritten)" }}>All-inclusive experiences at unbeatable value</p>
        </motion.div>
...
/* Desktop destination card — postcard / polaroid style with tilt */
function DestinationCard({ destination, index, isInView }: { destination: Destination; index: number; isInView: boolean }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 50, rotate: rotation }} animate={isInView ? { opacity: 1, x: 0, rotate: rotation } : {}} transition={{ duration: 0.6, delay: index * 0.15, ease: [0.22, 1, 0.36, 1] }} className="min-w-[320px] md:min-w-[380px] snap-center flex-shrink-0">
      <div ref={cardRef} onMouseMove={handleMouseMove} onMouseEnter={() => setHovered(true)} onMouseLeave={handleMouseLeave}
        ...
        <div className="absolute top-4 left-4 flex gap-2 z-10">
          {destination.tags.map((tag) => (
            <span key={tag} className="px-3 py-1 rounded-full border border-dashed border-white/20 text-[11px] text-cyan tracking-wider" style={{ fontFamily: "var(--font-handwritten)" }}>{tag}</span>
          ))}
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
          ...
          <motion.div animate={{ opacity: hovered ? 1 : 0.7, y: hovered ? 0 : 10 }} className="flex items-center justify-between">
            ...
            {/* Organic explore link — not a button */}
        ...
        {hovered && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 rounded-3xl pointer-events-none"
            style={{ boxShadow: "inset 0 0 0 1px rgba(0, 240, 255, 0.2), 0 0 40px rgba(0, 240, 255, 0.08)" }} />
        )}
      </div>
    </motion.div>
  );
...
/* Mobile destination card — immersive full-width */
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: index * 0.1, ease: [0.22, 1, 0.36, 1] }}>
      <div className="relative h-[380px] sm:h-[420px] rounded-3xl overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2 z-10">
          {destination.tags.map((tag) => (
            <span key={tag} className="px-2.5 py-1 rounded-full border border-dashed border-white/20 text-[11px] text-cyan" style={{ fontFamily: "var(--font-handwritten)" }}>{tag}</span>
          ))}
        </div>
...
/* Package card — journal entry style */
function PackageCard({ pkg, index, isInView }: { pkg: Package; index: number; isInView: boolean }) {
  return (
    <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.4 + index * 0.1, ease: [0.22, 1, 0.36, 1] }}
      className="glass rounded-3xl overflow-hidden group hover:border-cyan/15 transition-all duration-500">
        ...
        <div className="p-5 sm:p-6 flex-1">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-text-muted text-xs" style={{ fontFamily: "var(--font-handwritten)" }}>per person</p>
            </div>
          </div>
          ...
          {/* Organic CTA — flowing text, not a button */}
          <MagneticButton>
